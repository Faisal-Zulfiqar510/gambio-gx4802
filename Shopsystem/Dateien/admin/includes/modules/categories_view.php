<?php
/* --------------------------------------------------------------
   categories_view.php 2023-03-29
   Gambio GmbH
   http://www.gambio.de
...
if (isset($_GET['sorting'])) {
    ...
    $userConfigurationService->setUserConfiguration($userId, 'admin_product_sorting', $prodsort);
    $userConfigurationService->setUserConfiguration($userId, 'admin_category_sorting', $catsort);
} else {
    $adminProductSorting = $userConfigurationService->getUserConfiguration($userId, 'admin_product_sorting');
    $prodsort            = isset($adminProductSorting) ? $adminProductSorting : 'p.products_sort, p.products_name ASC';

    $adminCategorySorting = $userConfigurationService->getUserConfiguration($userId, 'admin_category_sorting');
    $catsort              =
        isset($adminCategorySorting) ? $adminCategorySorting : 'c.sort_order, cd.categories_name ASC';
}
...
function _getCaret($elementName)
{
    ...
    // By default the table is sorted by the name column
    if ($elementName === 'name' && !isset($_GET['sorting']) && !isset($adminProductSorting)) {
        $caretInformation['activeCaret'] = 'true';
    } elseif (isset($_GET['sorting']) && $_GET['sorting'] === $elementName . '-desc') {
        $caretInformation['sortingDirection'] = 'desc';
        $caretInformation['activeCaret']      = 'true';
    } elseif (isset($_GET['sorting']) && $_GET['sorting'] === $elementName) {
        $caretInformation['activeCaret'] = 'true';
    } elseif (strpos((string)$adminProductSorting, $elementName) !== false) {
        $caretInformation['activeCaret'] = 'true';
    }
    if (strpos((string)$adminProductSorting, 'DESC') !== false) {
        $caretInformation['sortingDirection'] = 'desc';
    }
    ...
    <tr>
        <td>
            <div class="pageHeading" style="float:left; background-image:url(html/assets/images/legacy/gm_icons/artkatalog.png)">
                ...
                if(isset($_GET['cPath']))
                {
                    ...
                    foreach ($categoryIds as $categoryId) {
                        ...
                        try {
                            ...
                            $categoryName    = $category->getName(new LanguageCode(new NonEmptyStringType($_SESSION['language_code'])));
                            $categoryNames[] = '<a href="' . GM_HTTP_SERVER . DIR_WS_ADMIN . 'categories.php?cPath='
                                               . implode('_', $categoryPath) . '" class="page-heading-link">'
                ...
                $existingCategoriesQuery  = 'SELECT COUNT(`categories_id`) as "categories_count" FROM `categories`';
                $existingCategoriesResult = $connection->query($existingCategoriesQuery)->fetch(FetchMode::ASSOCIATIVE);
                $existingCategoriesCount  = (int)$existingCategoriesResult['categories_count'];
                $createAction             = $existingCategoriesCount
                                            === 0 ? '&action=new_category' : '&action=new_product';
                ?>
            </div>
            ...
            <div class="gx-container create-new-wrapper">
                <div class="create-new-container pull-right">
                    ...
                    >
                        <button data-gx-extension="link" data-link-url="<?php echo xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'cPath=' . $cPath . $createAction) ?>" class="btn btn-success"><i class="fa fa-plus btn-icon"></i>
            ...
            <table data-gx-widget="table_sorting" border="0" width="100%" cellspacing="0" cellpadding="0">
                <tr>
                 ...
                 align="center" style="width: 85px">' . TEXT_STOCK . '</td>';
                            ...
                            if (xtc_not_null($_POST['multi_categories'] ?? null) || xtc_not_null($_POST['multi_products'] ?? null)) {
                            ...
                            if (!empty($_GET['search'])) {
                                ...
                                $categories_query = xtc_db_query("select c.categories_id, cd.categories_name, c.categories_image, c.parent_id, c.sort_order, c.date_added, c.last_modified, c.categories_status from " . TABLE_CATEGORIES . " c, " . TABLE_CATEGORIES_DESCRIPTION . " cd where c.parent_id = '" . $current_category_id . "' and c.categories_id = cd.categories_id and cd.language_id = '" . (int)($_SESSION['languages_id'] ?? null) . "' order by " . $catsort);
                            }
                            ...
                            while ($categories = xtc_db_fetch_array($categories_query)) {
                                ...
                                if ((empty($_GET['cID']) && empty($_GET['pID'])
                                     || ($_GET['cID'] ?? null) == $categories['categories_id'])
                                    && empty($cInfo)
                                    && substr($_GET['action'] ?? '', 0, 4) !== 'new_') {
                                ...
                                >
                                    <td class="categories_view_data"><input type="checkbox" class="checkbox" name="multi_categories[]" value="<?php echo $categories['categories_id'] . '" '; if (isset($_POST['multi_categories']) && is_array($_POST['multi_categories'])) { if (in_array($categories['categories_id'], $_POST['multi_categories'])) { echo 'checked="checked"'; } } ?>></td>
                     ...
                     AND p2c.categories_id = '" . $current_category_id . "'
                            ...
                            while ($products = xtc_db_fetch_array($products_query)) {
                                ...
                                if ((empty($_GET['pID']) && empty($_GET['cID']))
                                    || ($_GET['pID'] ?? null) == $products['products_id'] && empty($pInfo)
                                       && empty($cInfo)
                                       && substr($_GET['action'] ?? '',
                                                 0,
                                                 4) !== 'new_') {
                                ...
                                <tr class="dataTableRow visibility_switcher row_selection"
                                    ...
                                    <?php echo isset($_GET['page']) ? 'data-page="' . $_GET['page'] . '"' : '' ?>
                                >
                                    <td class="categories_view_data">
                                    ...
                                    >
                                        <a data-tooltip-trigger href="<?php echo xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) . 'cPath=' . $cPath . '&pID=' . $products['products_id']) . '&action=new_product'; ?>">
                                            <?php echo htmlspecialchars($products['products_name']); ?>
                                        </a>
                                    </td>

                                    <td class="categories_view_data">
                                        <?php
                                        echo htmlspecialchars($products['products_model']);
                                        ?>
                                    </td>
			...
			<tr class="gx-container no-hover">
				<td colspan="10" class="text-center">' . $gmLangEditTextManager->get_text('TEXT_NO_RESULT') . '</td>
                    ...
                    <?php  echo TEXT_CATEGORIES . ' ' . $categories_count; ?>
                                    ...
                                    // Back button
                                    if ($cPath) {
                                        echo '<span class="bottom-save-bar-content"><a class="btn btn-back hideable-control-element" style="display:inline-block;" onClick="this.blur()" href="' . xtc_href_link(FILENAME_CATEGORIES, xtc_get_all_get_params(array('cPath', 'action', 'pID', 'cID')) .  $cPath_back . '&cID=' . $current_category_id) . '"><i class="fa fa-fw fa-reply"></i> <span>' . BUTTON_BACK . '</span></a></span>';
                                    }
                                    ?>

                                    <!-- Go to -->
                                    <?php
                                    $query = xtc_db_query('SELECT count(*) AS `count` FROM `categories`;');
			...
			');
                                ...
                                if (isset($_POST['multi_products']) && is_array($_POST['multi_products'])) {
                                    foreach ($_POST['multi_products'] AS $multi_product) {
                                        ...
                                        $contents[] = array('text' => '<tr><td style="border-bottom: 1px solid Black; margin-bottom: 10px;" class="infoBoxContent"><b>' . htmlspecialchars(xtc_get_products_name($multi_product)) . '</b></td></tr>');
			...
			');
                                ...
                                if (isset($_POST['multi_categories']) && is_array($_POST['multi_categories'])) {
                                    foreach ($_POST['multi_categories'] AS $multi_category) {
                                        ...
                                        $cInfo_array = xtc_array_merge($category, $category_childs, $category_products);
                                        $cInfo = new objectInfo($cInfo_array);
                                        $contents[] = array('text' => '<tr><td style="border-bottom: 1px solid Black; margin-bottom: 10px;" class="infoBoxContent"><b>' . htmlspecialchars($cInfo->categories_name) . '</b></td></tr>');
                                    ...
                                    foreach ($_POST['multi_products'] AS $multi_product) {
                                        ...
                                        for ($i = 0, $n = sizeof($product_categories); $i < $n; $i++) {
                                            ...
                                            for ($j = 0, $k = sizeof($product_categories[$i]); $j < $k; $j++) {
                                                $category_path .= htmlspecialchars($product_categories[$i][$j]['text']) . '&nbsp;&gt;&nbsp;';
                                            }
                        ...
                        default:
                            if ($rows > 0) {
                                if (isset($cInfo) && is_object($cInfo)) {
                                    ...
                                    $heading[]  = array('align' => 'center', 'text' => '<b>' . htmlspecialchars(xtc_get_products_name($pInfo->products_id, $_SESSION['languages_id'])) . '</b>');
                                    //Multi Element Actions
                                    $contents[] = array('align' => 'center', 'text' => '<div style="padding-top: 5px; font-weight: bold;">' . TEXT_MARKED_ELEMENTS . '</div><br />');
                                    ...
                                    if (PRICE_IS_BRUTTO=='true' && ((isset($_GET['read']) && $_GET['read'] === 'only') || !isset($_GET['action']) || $_GET['action'] !== 'new_product_preview') ){
                                        $price_netto = xtc_round($price,PRICE_PRECISION);
