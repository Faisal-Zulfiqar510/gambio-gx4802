<?php
/* --------------------------------------------------------------
   additional_fields.inc.php 2022-07-29
   Gambio GmbH
   http://www.gambio.de
...
$query = 'SELECT additional_field_id FROM additional_fields WHERE item_type = "product" ORDER BY `additional_field_id`';
		...
		foreach($languages as $language)
		{
			...
			if(!empty($_GET['pID']))
			{
				...
				if(is_object($fieldValueObject))
				{
					...
					$fieldValue = $fieldValues[$language['id'] ?? null] ?? null;
				}
			}
