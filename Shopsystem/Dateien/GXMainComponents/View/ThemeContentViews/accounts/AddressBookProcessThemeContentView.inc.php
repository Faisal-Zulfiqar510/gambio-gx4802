<?php
/* --------------------------------------------------------------
  AddressBookProcessThemeContentView.inc.php 2020-09-02
  Gambio GmbH
  http://www.gambio.de
...
class AddressBookProcessThemeContentView extends ThemeContentView
{
    ...
    public function __construct()
    {
        ...
        $this->set_content_template('address_book_process.html');
        $this->set_flat_assigns(true);
        $this->coo_details_content_view = MainFactory::create_object('AddressBookDetailsThemeContentView');
    ...
    protected function set_validation_rules()
    {
        ...
        $this->validation_rules_array['action_delete']               = ['type' => 'bool'];
        ...
        $this->validation_rules_array['coo_details_content_view']    = [
            'type'        => 'object',
            'object_type' => 'AddressBookDetailsThemeContentView'
        ];
        $this->validation_rules_array['return_page']                 = ['type' => 'string'];
    }
    ...
    protected function add_form_data()
    {
        ...
        $this->content_array['return_page']     = (string)$this->return_page;
    }
    ...
    protected function add_delete_data()
    {
        ...
        $ImmutableClasses  = ['packstation', 'postfiliale', 'packstation_2', 'postfiliale_2', 'parcelshop'];
        ...
        $this->content_array['ADDRESS']            = xtc_address_label($this->customer_id,
                                                                       htmlentities_wrapper($this->coo_address->get_('address_book_id')),
                                                                       true,
                                                                       ' ',
                                                                       '<br />');
        
        $this->content_array['BUTTON_BACK_URL']   = xtc_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL');
        $this->content_array['BUTTON_DELETE_URL'] = xtc_href_link(FILENAME_ADDRESS_BOOK_PROCESS,
                                                                  'delete='
                                                                  . htmlentities_wrapper($this->coo_address->get_('address_book_id'))
                                                                  . '&action=deleteconfirm'
                                                                  . '&pageToken=' . $this->_generatePageToken(),
                                                                  'SSL');
    }
    ...
    protected function assign_data_to_details_content_view()
    {
        ...
        if (!empty($this->return_page) && array_key_exists($this->return_page, $validReturnPages)) {
            $backLink = xtc_href_link($validReturnPages[$this->return_page], '', 'SSL');
            $this->coo_details_content_view->set_('back_link', $backLink);
        }
    }
    ...
    {
        $this->content_array['BUTTON_BACK']   = '<a href="' . xtc_href_link(FILENAME_ADDRESS_BOOK,
                                                                            '',
                                                                            'SSL') . '">'
                                                . xtc_image_button('button_back.gif', IMAGE_BUTTON_BACK) . '</a>';
        $this->content_array['BUTTON_UPDATE'] = xtc_draw_hidden_field('action',
                                                                      'update') . xtc_draw_hidden_field('edit',
                                                                                                        htmlentities_wrapper($this->coo_address->get_('address_book_id')))
                                                . xtc_image_submit('button_update_cart.gif',
                                                                   IMAGE_BUTTON_UPDATE);
        
        $this->content_array['BUTTON_BACK_URL']             = xtc_href_link(FILENAME_ADDRESS_BOOK, '', 'SSL');
        $this->content_array['BUTTON_UPDATE_HIDDEN_FIELDS'] = xtc_draw_hidden_field('action',
                                                                                    'update')
                                                              . xtc_draw_hidden_field('edit',
                                                                                      htmlentities_wrapper($this->coo_address->get_('address_book_id')));
    }
    ...
    protected function add_new_data()
    {
        ...
        $this->content_array['BUTTON_BACK']   = '<a href="' . $back_link . '">' . xtc_image_button('button_back.gif',
                                                                                                   IMAGE_BUTTON_BACK)
                                                . '</a>';
        $this->content_array['BUTTON_UPDATE'] = xtc_draw_hidden_field('action',
                                                                      'process')
    ...
    private function _generatePageToken()
    {
        $cooPageToken = $_SESSION['coo_page_token'];
        if ($cooPageToken instanceof PageToken) {
            return $cooPageToken->generate_token();
        }
    }
}
