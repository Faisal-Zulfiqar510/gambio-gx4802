<?php
/*--------------------------------------------------------------
   ImageListWriter.php 2023-01-09
   Gambio GmbH
   http://www.gambio.de
...
class ImageListWriter
{
    ...
    private function removeImages(ImageList $imageList): array
    {
        ...
        if (count($relativePaths) !== 0) {
            ...
            $deleteImageQuery->andWhere($where = 'product_image_list_image_local_path NOT IN (' . implode(', ', $relativePaths) . ')');
            $deletedImagesQuery->andWhere($where);
        }
    
        $deletedImages = $deletedImagesQuery->execute()->fetchAll(FetchMode::NUMERIC);
        ...
        $this->connection->prepare($deleteImageTextQuery)->execute();
    ...
    private function getImageIdByRelativePathAndListId(int $id, Image $removedImage): int
    {
        ...
        return (int)$result->fetchAll()[0]['product_image_list_image_id'];
    }
    ...
    private function imageIsNotUsedInProductsOrLists(string $relativePath): ?string
    {
        ...
        $usage = $stmt->fetch(FetchMode::NUMERIC);
