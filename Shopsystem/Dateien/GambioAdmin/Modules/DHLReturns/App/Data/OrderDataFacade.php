<?php
/* --------------------------------------------------------------
   OrderDataFacade.php 2021-04-14
   Gambio GmbH
   http://www.gambio.de
...
class OrderDataFacade
{
    ...
    public function getOrderData(int $orderId): array
    {
        ...
        $orderStatement->bindParam('orders_id', $orderId, ParameterType::INTEGER);
        $orderStatement->execute();
        $orderRow = $orderStatement->fetch(FetchMode::ASSOCIATIVE);
        ...
        $shipmentStatement->bindParam('orders_id', $orderId);
        $shipmentStatement->execute();
        $shipmentRow    = $shipmentStatement->fetch(FetchMode::ASSOCIATIVE);
        ...
        $invoiceStatement->bindParam('order_id', $orderId);
        $invoiceStatement->execute();
        $invoiceRow    = $invoiceStatement->fetch(FetchMode::ASSOCIATIVE);
        ...
        $positionsStatement->bindParam('order_id', $orderId);
        $positionsStatement->execute();
        while ($positionsRow = $positionsStatement->fetch(FetchMode::ASSOCIATIVE)) {
            $positions[] = [
                'positionDescription' => $positionsRow['products_name'],
                'count'               => (int)$positionsRow['products_quantity'],
                'weightInGrams'       => 0,
                'values'              => (float)$positionsRow['products_price'],
                'originCountry'       => '',
                'articleReference'    => $positionsRow['products_model'],
                'tarifNumber'         => '',
            ];
        }
