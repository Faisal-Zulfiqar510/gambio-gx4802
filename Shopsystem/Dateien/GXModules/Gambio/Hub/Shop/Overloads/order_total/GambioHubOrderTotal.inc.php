<?php
/* --------------------------------------------------------------
   GambioHubOrderTotal.inc.php 2023-04-25
   Gambio GmbH
   http://www.gambio.de
                       ...
                       $GLOBALS[$class]->code === 'ot_gm_tax_free')
                    {
                        ...
                        if ($class === 'ot_total' && strpos($_SERVER['REQUEST_URI'], 'checkout_process.php') !== false) {
                            $sessionKeyExists = array_key_exists('checkout_interest', $_SESSION)
                                ...
                                && array_key_exists('gambio_hub_session_key', $_SESSION['checkout_interest']);
                            if ($sessionKeyExists && !empty($_SESSION['checkout_interest'][$_SESSION['gambio_hub_session_key']])
                                && (strpos($_SERVER['REQUEST_URI'] ?? '', 'checkout_confirmation') !== false
                                    || strpos($_SERVER['REQUEST_URI'] ?? '', 'checkout_process') !== false)) {
