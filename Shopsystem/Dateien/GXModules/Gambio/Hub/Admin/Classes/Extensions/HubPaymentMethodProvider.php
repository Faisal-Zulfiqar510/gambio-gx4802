<?php
/* --------------------------------------------------------------
   HubPaymentMethodProvider.php 2023-02-21
   Gambio GmbH
   http://www.gambio.de
...
class HubPaymentMethodProvider
{
    ...
    public static function getPaymentMethods(Connection $db): array
    {
        ...
        foreach (self::getModules($db) as $moduleName) {
            ...
            ";
            $text  = (string)$db->fetchAssoc($query)['value'];
            ...
            $moduleName = $db->fetchAssoc($query)['gambio_hub_module'] ?? $moduleName;
    ...
    private static function getModules(Connection $db): array
    {
        ...
        $hubAliases  = $db->fetchAllAssociative($query);
        ...
        $forbiddenDownloadPaymentMethods = (string)$db->fetchAssoc($query)['value'] ?? '';
        $forbiddenDownloadPaymentMethods = preg_split('/\s*,\s*/', $forbiddenDownloadPaymentMethods);
