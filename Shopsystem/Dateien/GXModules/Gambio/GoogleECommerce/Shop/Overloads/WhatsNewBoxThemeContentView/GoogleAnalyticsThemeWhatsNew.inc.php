<?php
/* --------------------------------------------------------------
   GoogleAnalyticsThemeWhatsNew.inc.php 2022-10-27
   Gambio GmbH
   http://www.gambio.de
...
/**
 * Class representing the what's new box content view overload for Google Analytics
 */
class GoogleAnalyticsThemeWhatsNew extends GoogleAnalyticsThemeWhatsNew_parent
{
    public function prepare_data()
    {
        ...
        if (!$isEnabled || !$isTrackingEnabled || !array_key_exists('box_content', $this->get_content_array()) || !$hasId) {
            return;
        }
        ...
        $data = $trackingService->encodedImpressionByProductId(new IdType($this->get_content_array()['box_content']['PRODUCTS_ID']),
                                                               ...
                                                               new NonEmptyStringType($configReadService->listName(GoogleAnalyticsImpressionType::boxWhatsNew())));
