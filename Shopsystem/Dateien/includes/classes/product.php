...
class product_ORIGIN implements ProductDataInterface
{
	...
	public function __construct($pID = 0, $p_language_id = false)
	{
		...
		{
			$t_language_id = (int)($_SESSION['languages_id'] ?? null);
		}
		else
		{
    ...
    function getDefaultShippingStatus($productId = null)
    {
        ...
        $t_query = 'SELECT
					...
					WHERE
						...
						shipstat.language_id = ' . (int)($_SESSION['languages_id'] ?? null);
	...
	function getReviewsCount()
	{
		$t_query = 'SELECT
					...
					WHERE
						...
						AND rd.languages_id = "' . (int)($_SESSION['languages_id'] ?? null) . '"
						AND rd.reviews_text != ""';
		...
		if($xtPrice->xtcCheckSpecial($this->pID) == 0)
		{
			...
			for($i = 0, $n = sizeof($staffel); $i < $n; $i ++)
			{
                ...
                if ($xtPrice->cStatus['customers_status_show_price_tax'] !== '0') {
                    $taxRate = $xtPrice->TAX[$this->data['products_tax_class_id'] ?? null] ?? 0;
                } else {
                    $taxRate = 0;
                }
	...
	function getVPEtext($product, $price)
	{
		...
		if(($product['products_vpe_status'] ?? null) == 1 && ($product['products_vpe_value'] ?? null) != 0.0 && $price > 0)
		{
    ...
    /**
     * @deprecated VPE support for attributes was removed in GX 4.7
     *
     * @param int $productId
	...
	function buildDataArray(&$array, $image = 'thumbnail')
	{
		...
		$tax_rate = $xtPrice->TAX[$array['products_tax_class_id'] ?? 0] ?? 0.0;

		$coo_properties_control = MainFactory::create_object('PropertiesControl');
		$t_combi = $coo_properties_control->get_cheapest_combi($array['products_id'], $_SESSION['languages_id']);
        
        $products_price = $xtPrice->xtcGetPrice($array['products_id'], true, 1, $array['products_tax_class_id'], $array['products_price'], 1, 0, true, true, $t_combi['products_properties_combis_id'] ?? false, true);
		...
		{
            $languageTextManager = MainFactory::create_object('LanguageTextManager', ['general']);
			...
			if($_SESSION['customers_status']['customers_fsk18_purchasable'] === '0')
			{
				if($array['products_fsk18'] == '0')
				{
					...
					$gm_buy_now .= xtc_image_submit('button_in_cart.gif', $languageTextManager->get_text('IMAGE_BUTTON_IN_CART'), 'class="gm_image_button"');
		...
		if((int)$this->getPropertiesCount($array['products_id']) > 0 && array_key_exists('use_properties_combis_shipping_time', $array)
           && $array['use_properties_combis_shipping_time'] != 0)
		{
    ...
    public function productImageSize($productId, $image)
    {
        $imageSize = [0, 0];
        if (file_exists(DIR_FS_CATALOG . DIR_WS_THUMBNAIL_IMAGES . $image)) {
            $imageSize = @getimagesize(DIR_FS_CATALOG . DIR_WS_THUMBNAIL_IMAGES . $image);
            xtc_db_query('UPDATE `products`
                            SET
                                `products_image_w` = ' . (int)$imageSize[0] . ',
                                `products_image_h` = ' . (int)$imageSize[1] . '
    ...
    {
        return (string)$this->data['products_model'];
    }
