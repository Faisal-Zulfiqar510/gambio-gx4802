...
/* --------------------------------------------------------------
 ...
 class main_ORIGIN {
	 ...
	 public function getShippingLink($p_lightbox = false, $p_productId = 0)
	{
		...
		{
            $languageTextManager = MainFactory::create_object('LanguageTextManager', array('product_info', (int)($_SESSION['languages_id'] ?? null)));
            ...
            $shipping = $languageTextManager->get_text('SHIPPING_COSTS', 'general');
            
			return ' '.$languageTextManager->get_text('SHIPPING_EXCL', 'general').'
	...
	function getTaxInfo($tax_rate) {
		...
		{
			$tax_info = $languageTextManager->get_text('GM_TAX_FREE');
		}
		else
		{
			...
			{
				$tax_info = sprintf($languageTextManager->get_text('TAX_INFO_INCL'), $tax_rate . '%');
			}
			// excl tax + tax at checkout
			if(($tax_rate > 0 || (gm_get_conf('DISPLAY_0_PROCENT_TAX') === '1' && $tax_rate >= 0 && $tax_rate !== null))
			   && $_SESSION['customers_status']['customers_status_show_price_tax'] == 0
			...
			{
				$tax_info = sprintf($languageTextManager->get_text('TAX_INFO_ADD'), $tax_rate . '%');
			}
			// excl tax
			if(($tax_rate > 0 || (gm_get_conf('DISPLAY_0_PROCENT_TAX') === '1' && $tax_rate >= 0 && $tax_rate !== null))
			   && $_SESSION['customers_status']['customers_status_show_price_tax'] == 0
			...
			{
				$tax_info = sprintf($languageTextManager->get_text('TAX_INFO_EXCL'), $tax_rate . '%');
			}
	...
	public static function get_abroad_shipping_info_link()
	{
		...
		$t_query = 'SELECT content_title
					...
					WHERE
						...
						languages_id = ' .  (int)($_SESSION['languages_id'] ?? null);
