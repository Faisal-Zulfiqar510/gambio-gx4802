<?php
/* --------------------------------------------------------------
   order_total.php 2022-05-02 gambio
   Gambio GmbH
   http://www.gambio.de
...
class order_total_ORIGIN {
     ...
     * @return string
     *
     * @deprecated credit selection on checkout_payment is no longer supported, auxiliary payments MUST be selected in
     *             the shopping cart!
     */
	...
	function update_credit_account($i) {
		if (MODULE_ORDER_TOTAL_INSTALLED) {
			...
			foreach($this->modules as $value) {
				...
				if (array_key_exists($class, $GLOBALS) && ((property_exists($GLOBALS[$class], 'enabled') && $GLOBALS[$class]->enabled) && (property_exists($GLOBALS[$class], 'credit_class') && $GLOBALS[$class]->credit_class))) {
